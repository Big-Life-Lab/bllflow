...
This is done by comparing it agains the modules.csv
...
and would simply output warnings and a boolean representing the valitity of the targets with the module.
...
The **targets_source** would be identical to the one in verify targets
...
The function would first run verify targets to confirm correct order and presense of steps. Then it would modify the tar_targets arguments to reflect their true value rather then the shorthand(roles).
Once the tar_targets were modifed accordingly the _targets.R file is written and tar_make() is ran letting targets handle the returns and the pipeline
...
# Contents of the modules.csv

### Step_id
The step_id column must contain a unique identifier for the step being performed

*Note* If we wish to have multiple modules.csv being present for 1 targets execution these names must be unique accross all csv as long as they belong to the same targets execution **I dont reccoment this approach**
...
*Note* If we wish to support being able to pipe functions within a step this could be done by listing the functions name in the same cell with a seperator, same with the function args further down the line. However I do not see a clean way of implementing piping order therefore if we do include piping it must be written in the order it is ran

### Step_arguments
The step_arguments column would contain the arguments for the function including our shorthand(roles). I belive we should bring back the roles() and formula notations from previous implementation to avoid any confusion later

*Note* For piping the arguments would be wrapped in () and then seperated with the same seperator as step_function
...
# Potentially optional columns

### Study name
Purely for documentation purposes to distinguish the modules.csv appart if the file name is not enough

### Start Databases
List of started databases in case variables and variable details is lost can be used to match. Alternatively this can be used to improve performance during the reading/initializing bllflow by disregarding databases present in the config in case the user loaded the wrong config or did not fill out their profile data fully.
