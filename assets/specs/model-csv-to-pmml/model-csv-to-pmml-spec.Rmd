...
One of the four mandatory files that the user has to provide. Each row in this CSV file gives the location of a file needed for implementation. The columns for this file are defined below:

* **fileType**: Used to identify what file this row is referring to
    * **type**: enum
    * **allowable values**
        * **variables**: Value for the variables sheet. There has to be an entry in this file with this value.
        * **variable-details**: Value for the variable details sheet. There has to be an entry in this file with this value.
        * **model-steps**: Value for the model steps sheet. There has to be an entry in this file with this value.
    * **restrictions**
        **mandatory**
* **filePath**: The relative path to this file. The path should be relative to the model export file
    * **type**: string
    * **restrictions**
        **mandatory**
...
* The third row gives the location of the model steps file. It's located in the same folder as the model export fiel and is called `model-steps.csv`
...
3. **centeredVariable**: The name of the new centered variable
    * **type**: string
4. **centeredVariableType**: Type of the new centered variable
    * **type**: enum
    * **allowable values**
        **cat**: Categorical
        **cont**: Continuous
...
2. **interactionVariable**: The name of the new interaction variable
    * **type**: string
3. **interactionVariableType**: The type of the new interaction variable
    * **type**: enum
    * **allowableValues**
        * **cat**: For categorical variables
        * **cont**: For continuous variables
...
2. **coefficient**: The value of the beta coefficient for the covariate
    **type**: number
3. **type**: Whether the covariate is categorical or continuous
    * **type** enum
    * **allowable values**:
      * **cat**: For categorical covariates
      * **cont**: For continuous covariates
...
For each new centered variable defined in the CSV file, we have to add a `DerivedField` node to the `LocalTransformations` node within the `PMML` node. 
...
And finally the XML nodes that needs to be added to the `LocalTransformations` node looks like,

```{xml}
<!-- The XML for the age_C variable-->
<DerivedField name="age_C">
  <Apply function="-">
    <FieldRef field="age" />
...
<DerivedField name="diet_score_C">
  <Apply function="-">
...
1. For each new centered variable defined in the CSV file (i.e. for each new row), a `DerivedField` field node needs to be added to the `LocalTransformations` node within the `PMML` node
2. The `name` attribute for the `DerivedField` node should be the value of the `centeredVariable` column for that row
3. The value of the `field` attribute for the `FieldRef` should be the value of the `origVariabvle` column for that row
4. The value within the `Constant` node should the the value of the `centerValue` column for that row

### RCS Step

Converting an RCS step file to PMML involves adding `DerivedField` nodes to the `LocalTransformations` node within the `PMML` node. Things to note:
...
4. For the first RCS variable in each row, the contents of the `DerivedField` node should just set it to the variable in the `variable` column
5. For the other RCS variables, the contents of each `DerivedField` node should call a function called `rcs` whose signature looks like,

```{r}
# previous_rcs_variable: The value in the previous rcs variable
...
<DerivedField name="AgeC_rcs1" optype="continuous">
  <Apply function="equal">
    <FieldRef field="Age_c" />
  </Apply>
</DerivedField>
  
<DerivedField name="AgeC_rcs2" optype="continuous">
  <Apply function="rcs">
...
<DerivedField name="AgeC_rcs3" optype="continuous">
  <Apply function="rcs">
    <FieldRef field="AgeC_rcs2" />
    <Constant dataType="float">3</Constant>
    <Array n="5" type="float">-11.5 -6.5 -1.5 5.5 16.5</Array>
  </Apply>
</DerivedField>
  
<DerivedField name="AgeC_rcs4" optype="continuous">
  <Apply function="rcs">
    <FieldRef field="AgeC_rcs3" />
    <Constant dataType="float">4</Constant>
    <Array n="5" type="float">-11.5 -6.5 -1.5 5.5 16.5</Array>
  </Apply>
</DerivedField>
...
<DerivedField name="PackYearsC_rcs1" optype="continuous">
  <Apply function="equal">
    <FieldRef field="PackYears_c" />
  </Apply>
</DerivedField>
  
<DerivedField name="PackYearsC_rcs2" optype="continuous">
  <Apply function="rcs">
...
Converting an interaction step file to PMML involves adding a `DerivedField` field for each new interaction variable (each row in the CSV file) to the `LocalTransformations` node within the `PMML` node. Things to note:

1. Add a `DerivedField` node for each new interaction variable 
2. The `name` attribute for the `DerivedField` field is the name of the interaction variable it's for
...
<DerivedField name="AgeXHypertension">
  <Apply function="*">
    <FieldRef field="Age" />
...
![](./fine-and-gray-step.png)
Things to note:

1. The following attributes need to be added to the `GeneralRegressionNode`
...
2. The following attributes need to be set for each `MiningField` nodes
    ...
    * `usageType`: Set to `active`. For the extra node added for the predicted risk, the value would be `target` and for the extra node added for the `time` variable, the value would be `input`
...
<GeneralRegressionModel
	...
	<MiningSchema>
	  ...
	  <MiningField name="time" usageType="time" />
