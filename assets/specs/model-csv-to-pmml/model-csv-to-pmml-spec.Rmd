...
3. For the first RCS variable in each row, the contents of the `DerivedField` node should just set it to the variable in the `variable` column
4. For the other RCS variables, the contents of each `DerivedField` node should call a function called `rcs` whose signature looks like,
...
<DerivedField name="AgeC_rcs1">
  <Apply function="equal">
    <FieldRef field="Age_c" />
  </Apply>
</DerivedField>
  
<DerivedField name="AgeC_rcs2">
  <Apply function="rcs">
...
<DerivedField name="PackYearsC_rcs1">
  <Apply function="equal">
    <FieldRef field="PackYears_c" />
  </Apply>
</DerivedField>
  
<DerivedField name="PackYearsC_rcs2">
  <Apply function="rcs">
...
2. The following attributes need to be set for each `MiningField` nodes
    * `name`: Set to the name of the starting variable. For the extra node added for the predicted risk, the value would be `risk`
    * `usageType`: Set to `active`. For the extra node added for the predicted risk, the value would be `target`
