...
```
## Introduction
**bllflow** uses seven steps to create a model. There is an emphasis on a _pre-specified_ approach to analysis. Pre-specified means 
...
_bllflow_ uses general utility functions for data cleaning and variable transformation. Functions for analysis logs and managing variable labels and other metadata are also included. The utility functions can be used by themselves -- even if you don't want to follow _bllflow_'s prespecified approach. When you are ready, you can use the Model Specification Workbook to describe your data cleaning and transformation steps -- additional bllflow 'wrapper' functions execute the utility functions according the Model Specification Workbook.

### Step 1 - Pre-specifying your model
Model specifications are recorded in a series of four CVS worksheets within 
the Model Specification Workbook (MSW). The most important sheet is the `variables` worksheets -- a CSV file with a row for each variable in your model. Included are columns for variable labels, data cleaning, variable 
 transformations and other instructions (Step 4).

We've found that people starting a new project like using the MSW even if they don't use other parts of _bllflow_. The MSW worksheet helps organize your thoughts about what variables to include in your model. The worksheets are also helpful when working in teams. We get everyone invovled in the MSW: analysts, methodologists and content experts. Graduate students have a team too -- you can use the MSW to review your modelling plans with your supervisor and thesis committee.
