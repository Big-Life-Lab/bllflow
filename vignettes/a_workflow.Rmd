...
known as variables, risk factors or features) and the outcome (target). All analyses are logged, 
allowing transparent reporting. 

### Step 1 - Pre-specifying your model
The model specifications are recorded within a series of 4 CVS worksheets that 
together form the Model Specification Workbook (MSW). You can use bllFlow without 
a MSW but we recommend using atleast the `variables` and `variable_details` worksheets. 

The **variables.csv** worksheet is the worksheet, with each representing a variable
 in your model. Included are columns for variable labels, data cleaning, variable 
...
or population and additional tables and figures. With **bllFlow**, you specify what 
variables are required for each table and then use utilitie functions to create 
the tables. 
...
This step is short and breif for **bllFlow**. **bllFlow**
 ...
 Rather, **bllFlow** is a wrapper around the model model derivation, 
 ...
 In **bllFlow**, dummy variables and other transformations are generated prior 
 to function call.
