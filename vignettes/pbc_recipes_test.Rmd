...
##Process the data
@Doug doing b-spline basis on protime causes an error not sure why. Is it something that can have this spline per
```{r}
pbcRec <- pbcRec %>%
  ...
  step_bs(bili,albumin,protime,match("edema"))
...
```{r}
coxPbcModel <- coxph(Surv(status) ~ age+sex+bili+albumin+protime+edema, data = bakedPBC)
coxPbcModel
```