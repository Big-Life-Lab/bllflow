...
### Variable roles
`bllflow` can assign one or more roles to each variable in the data (i.e., variables can have multiple roles). We add a role for each variable that is used in our study. You can define your own role or use suggested roles below. You can use or reuse roles for any purpose, such as using as building a model using the role 'predictors' as a selection, or generate Table 1 using a role 'table-1'.
