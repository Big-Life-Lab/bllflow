...
The model the we will develop has six variables or predictors: age, sex, bili, albumin, protime, edema. 

###### Example 1: MSW variables
...
variablesCSV <- read.csv(system.file("extdata", "PBC/PBC-variables.csv", package = "bllFlow"))
datatable(variablesCSV, options = list(pageLength = 6))
```

## How to create the MSW sheets (and use existing metadata)
...
###### Example 2: MSW variable details
...
The `variableDetails` sheet also includes transformed variables used throughout the study. In our example model, we use age as a non-linear predictor (3 know restricted cubic spline). However, `Table 1` and other tables report `age` categories. We added the transformed `age_cat4` variable to the `variableDetails.csv` file, along with labels and infomiation on the age range for each category. 

There are 16 rows in `variableDetails`. We included `age` as the first example, with the remaining rows representing only the newly transformed variables -- variables that do not existing our orginal `pbc` data. The information for variables in the original `pbc` data are in the `pbcDDI.xml` file. That metadata can be added with the DDI utility functions describe later [Helper and utility functions](vignettes/i_helper_functions.Rmd)

```{r}
variablesDetailsCSV <- read.csv(system.file("extdata", "PBC/PBC-variableDetails.csv", package = "bllFlow"))
datatable(variablesDetailsCSV, options = list(pageLength = 5))
```
