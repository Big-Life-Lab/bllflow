...
### Variabe roles
`bllflow` can assign one or more roles to each variable in the data. We add a role for each variable that is used in our study. Suggested can be named anything and be used or reused for any purpose. Variables can have multiple roles. Suggested roles include:
...
- strata - used to stratify the development of a mode. e.g. create two different models for males and females, or split the sample into development and validation sub-samples.
- descriptor - used to describe a study population such as in Table 1 or 2, or report the performance of a model
- intermediate -  to only to creating other (derived) variables
