...
## Workflow vignettes
The [`Workflow`](a_workflow.html) vignettes use the `pbc` data available in the `survival` package to [replicate](https://www.ncbi.nlm.nih.gov/pubmed/2737595) a survival model for people with primary biliary cirrhosis.  What is the `pbc` data? The name, description and other information is included in the metadata file! 

See [`Example 4 - Helper and utility functions`](i_helper_functions.html). 

## Example - Study exclusion criteria
A typical first step when starting a new study is applying inclusion and exclusion criteria to the study data. In our PBC survival model, we only include participants aged 40 to 70 years.
