      ...
      recipe_object <-
        do.call(get(step_name),
                list(recipe = recipe_object,
                     rlang::parse_expr(unlist(step_formula))))
    }
    recipe_object <-
...
create_formula_variable_formula <- function(functions) {
  ...
  for (variable in names(functions[[
    pkg.globals$FunctionList.VariableArguments]])) {
    return_formula <- append(return_formula,
                             paste(variable,
                                   functions[[
                                     pkg.globals$FunctionList.Arguments]],
                                   ",", sep = ""))
  }
  return_formula[[length(return_formula)]] <-
    stringr::str_sub(return_formula[[length(return_formula)]], end = -2)
