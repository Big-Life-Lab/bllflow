...
clean.Min.BLLFlow <- function(bllFlowModel, print = FALSE) {
  bllFlowModel<-ProcessMinOrMax(bllFlowModel, "min")
}
...
clean.max.BLLFlow <- function(bllFlowModel, print = FALSE) {
  bllFlowModel<- ProcessMinOrMax(bllFlowModel, "max")
}

# Internal helper functions ------------------------------------------------------------------

# Create a list of only the variables that need min applied to them
CreateOperationVariableList <- function(variable, operation, outlier) {
  if (!is.na(operation)) {
...
ProcessMinOrMax <- function(bllFlowModel, operation) {

  # This is to only store rows which contain instructions for Min
  variablesToCheck <- apply(bllFlowModel$variables, 1, function(y) CreateOperationVariableList(y["variable"], y[operation], y["outlier"]))
  ...
  for (i in variablesToCheck) {
    ...
    if (i["outlier"] == "delete") {
      if (as.character(operation) == "min") {
        ...
        bllFlowModel$data <- bllFlowModel$data[!(bllFlowModel$data[i["variable"]] < i[operation] & !is.na(bllFlowModel$data[i["variable"]])), ]
    ...
    # Handle missing outlier
      ...
      preContainRows <- length(which(is.na(bllFlowModel$data[i["variable"]])))
      
      if (as.character(operation) == "min") {
        bllFlowModel$data[i["variable"]][bllFlowModel$data[i["variable"]] < i[operation]] <- NA
      } else if (as.character(operation) == "max") {
        bllFlowModel$data[i["variable"]][bllFlowModel$data[i["variable"]] > i[operation]] <- NA
      }
      
      postRows <- length(which(is.na(bllFlowModel$data[i["variable"]])))
      affectedRows <- postRows - preContainRows
    
    # Handle the replace with outlier
    } else if (!is.na(as.numeric(i["outlier"]))) {
      preContainRows <- length(which(bllFlowModel$data[i["variable"]] == i["outlier"]))
      if (as.character(operation) == "min") {
        bllFlowModel$data[i["variable"]][bllFlowModel$data[i["variable"]] < i[operation]] <- i["outlier"]
      } else if (as.character(operation) == "max") {
        bllFlowModel$data[i["variable"]][bllFlowModel$data[i["variable"]] > i[operation]] <- i["outlier"]
      }
      postRows <- length(which(bllFlowModel$data[i["variable"]] == i["outlier"]))
      affectedRows <- postRows - preContainRows
    ...
    bllFlowModel <- LogFunctionActivity(bllFlowModel, preRows, affectedRows, i["outlier"], paste(i["variable"], " ", operation, " at ", i[operation], sep = ""), paste("clean.", operation, ".BLLFlow", sep = ""),i[["variable"]],i[[operation]])
  }
  return(bllFlowModel)
}
