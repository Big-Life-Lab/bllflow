...
#'
#' @examples:
#'   The messyEducation_cat variable has 8 categories including the missing category
#'   The messyDiabetes_bin variable is a binary variable with a separate category for missings
                                  ...
                                  datasetname)
{
  ...
  prevalence<-aggregate(da$percent ~ addNA(GroupByName1)  +addNA(GroupByName2),
                                    datasetname, FUN=sum, na.rm=FALSE, na.action = na.exclude)
