  ...
  function(frameToPopulateFrom,
    ...
    for (nameIndex in 1:length(names(labeledVariables))) {
      ...
      for (rowToCheck in 1:nrow(rowsToCheck)) {
        ...
        } else if (!is.null(labeledVariables[[nameBeingChecked]][[nameBeingChecked]]$variableStartType) & !is.na(rowsToCheck[rowToCheck, pkg.globals$argument.VariableStartType])) {
          if (rowsToCheck[rowToCheck, pkg.globals$argument.VariableStartHigh] == labeledVariables[[nameBeingChecked]][[nameBeingChecked]][[pkg.globals$argument.VariableStartHigh]] &
              rowsToCheck[rowToCheck, pkg.globals$argument.VariableStartLow] == labeledVariables[[nameBeingChecked]][[nameBeingChecked]][[pkg.globals$argument.VariableStartLow]]) {
            # Populate every column with values pulled from DDI
            for (columnName in names(labeledVariables[[nameBeingChecked]][[as.character(nameBeingChecked)]])) {
              # Check if there is any data precent in the cell in order to not override anything
              if (CheckIfCellIsEmpty(rowsToCheck[rowToCheck, columnName],
                                     rownames(rowsToCheck)[rowToCheck],
                                     columnName,
                                     labeledVariables[[nameBeingChecked]][[as.character(nameBeingChecked)]][[columnName]])) {
                # If this has not been in the dataframe upon creation that level is added
                if (!labeledVariables[[nameBeingChecked]][[as.character(nameBeingChecked)]][[columnName]] %in% levels(rowsToCheck[, columnName])) {
                  levels(rowsToCheck[, columnName]) <-
                    c(levels(rowsToCheck[, columnName]), labeledVariables[[nameBeingChecked]][[as.character(nameBeingChecked)]][[columnName]])
                }
                rowsToCheck[rowToCheck, columnName] <-
                  labeledVariables[[nameBeingChecked]][[as.character(nameBeingChecked)]][[columnName]]
              }
            ...
            # Remove that value from the list to avoid repetition during new row creation
            labeledVariables[[nameBeingChecked]][[nameBeingChecked]] <-
              NULL
            finalFrame <- rbind(finalFrame, rowsToCheck[rowToCheck, ])
          }
        } else{
...
#' Write populated variable details sheet
#'
#' @param ddi the path to the ddi file containing the necessary information
#' @return returns a list containg he necissary data
#' @export
