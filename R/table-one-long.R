...
CreateTableOne.BLLFlow <- function(x,
  ...
  dropped_vars <- vars[!(vars %in% colnames(bll_flow_model[[pkg.globals$bllFlowContent.WorkingData]]))]
  if (length(dropped_vars)>0){
    message(paste(dropped_vars, "were not present in working data"))
  }
  dropped_factor <- factor_vars[!(factor_vars %in% colnames(bll_flow_model[[pkg.globals$bllFlowContent.WorkingData]]))]
